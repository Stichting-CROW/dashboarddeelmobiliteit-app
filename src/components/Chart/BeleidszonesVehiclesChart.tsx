...
interface BeleidszonesVehiclesChartProps {
  title?: string;
}

function sumValuesPerBucket(
  ...
  values.forEach((item) => {
    ...
    Object.keys(item).forEach((key) => {
      if (key !== timeKey && key !== 'start_interval') {
        const val = item[key];
...
function BeleidszonesVehiclesChart({ title }: BeleidszonesVehiclesChartProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [hubData, munData] = await Promise.all([
          getZoneAvailabilityStats(token, {
            zoneIds,
            startTime,
            endTime,
            aggregationLevel,
          ...
          getAggregatedStats(token, 'available_vehicles', {
            filter: { ...filter, zones: '' },
            metadata,
            aggregationLevel,
        ...
        const hubValues = hubData?.availability_stats?.values || [];
        const munValues =
          munData?.available_vehicles_aggregated_stats?.values ||
          munData?.availability_stats?.values ||
          [];
        const hubTimeKey = hubValues[0]?.time !== undefined ? 'time' : 'start_interval';
        const munTimeKey = munValues[0]?.time !== undefined ? 'time' : 'start_interval';

        const hubTotals = sumValuesPerBucket(hubValues, hubTimeKey);
        const munTotals = sumValuesPerBucket(munValues, munTimeKey);
        ...
        if (!cancelled) {
          console.error('BeleidszonesVehiclesChart error:', err);
...
export default BeleidszonesVehiclesChart;
